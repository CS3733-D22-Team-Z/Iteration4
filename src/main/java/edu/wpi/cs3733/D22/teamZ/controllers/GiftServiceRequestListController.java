...
public class GiftServiceRequestListController implements Initializable, IMenuAccess {
  ...
  private final String requestPageURL =
      "edu/wpi/cs3733/D22/teamZ/views/GiftServices.fxml"; // change

  // List of identifiers for each
  private final String[] identifiers = {
    "ID", "Gift Type", "Issuer", "Handler", "Status", "Target Location", "Patient" // change
  };

  // Columns to be represented by the table
  private final List<String> visibleColumns =
      List.of("ID", "Gift Type", "Status", "Patient", "Issuer"); // change
  ...
  private List<GiftServiceRequest> rawRequests; // change
  ...
  public GiftServiceRequestListController() throws SQLException { // change
  ...
  public String getMenuName() {
    return "Gift Service Request List";
  } // change
  ...
  public void createRRList() {
    ...
    for (GiftServiceRequest request : rawRequests) { // change
      ...
      requests.add(
          new RequestRow(
              ...
              detailRetrievers.get(3).call(request)));
    }
  ...
  public void loadRow(String reqID) {
    ...
    GiftServiceRequest selectedReq = getRequestFromID(reqID); // change
  ...
  public void loadRequests() throws SQLException {
    rawRequests = FacadeDAO.getInstance().getAllGiftRequests(); // change
  }

  public GiftServiceRequest getRequestFromID(String MeqID) { // change
    return FacadeDAO.getInstance().getGiftServiceRequestByID(MeqID); // change
  }
  ...
  public void exportToCSV(ActionEvent actionEvent) {
    ...
    File defaultFile = facadeDAO.getDefaultGiftServiceCSVPath(); // change
    ...
    facadeDAO.exportGiftRequestToCSV(file); // change
  }
  ...
  private static class RequestRow {
    ...
    SimpleStringProperty giftType;
    SimpleStringProperty issuer;
    SimpleStringProperty status;
    SimpleStringProperty patient;

    public RequestRow(
        String newId, String newType, String newIssuer, String newStatus, String patientName) {
      id = new SimpleStringProperty(newId);
      giftType = new SimpleStringProperty(newType);
      issuer = new SimpleStringProperty(newIssuer);
      status = new SimpleStringProperty(newStatus);
      patient = new SimpleStringProperty((patientName));
    }
  ...
  private interface RequestFunc {
    String call(GiftServiceRequest request); // change
  }
}
