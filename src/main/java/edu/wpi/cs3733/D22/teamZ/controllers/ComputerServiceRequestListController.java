...
public class ComputerServiceRequestListController implements Initializable, IMenuAccess {
  ...
  private final String requestPageURL =
      "edu/wpi/cs3733/D22/teamZ/views/ComputerServiceRequest.fxml"; // change

  // List of identifiers for each
  private final String[] identifiers = {
    "ID", "OS", "Issuer", "Handler", "Status", "Target Location", "Problem" // change
  };

  // Columns to be represented by the table
  private final List<String> visibleColumns = List.of("ID", "OS", "Status", "Issuer"); // change

  // Retriever functions. Correspond to visible columns.
  private final List<RequestRowFunc> retrievers =
      List.of(row -> row.id, row -> row.OS, row -> row.status, row -> row.issuer);

  private final List<RequestFunc> detailRetrievers =
      List.of(
          request -> request.getRequestID(),
          request -> request.getOperatingSystem(),
          ...
          request -> request.getProblemDescription());

  // List of requests that represents raw data
  private List<ComputerServiceRequest> rawRequests; // change
  ...
  public ComputerServiceRequestListController() throws SQLException { // change
  ...
  public void createRRList() {
    ...
    for (ComputerServiceRequest request : rawRequests) { // change
  ...
  public void loadRow(String reqID) {
    ...
    ComputerServiceRequest selectedReq = getRequestFromID(reqID); // change
  ...
  public void loadRequests() throws SQLException {
    rawRequests = FacadeDAO.getInstance().getAllComputerServiceRequests(); // change
  }

  public ComputerServiceRequest getRequestFromID(String MeqID) { // change
    return FacadeDAO.getInstance().getComputerServiceRequestByID(MeqID); // change
  }
  ...
  public void exportToCSV(ActionEvent actionEvent) {
    ...
    File defaultFile = facadeDAO.getDefaultComputerRequestCSVPath(); // change
    ...
    facadeDAO.exportToComputerServiceRequestCSV(file); // change
  }
  ...
  private static class RequestRow {
    ...
    public RequestRow(String newId, String newType, String newIssuer, String newStatus) {
      ...
      OS = new SimpleStringProperty(newType);
  ...
  private interface RequestFunc {
    String call(ComputerServiceRequest request); // change
  }
}
