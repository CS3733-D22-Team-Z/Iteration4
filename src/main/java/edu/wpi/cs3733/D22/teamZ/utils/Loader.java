package edu.wpi.cs3733.D22.teamZ.utils;
...
public abstract class Loader {
        ...
        switch (line.charAt(0)) {
          ...
          case 'f':
            ...
            for (int y = 0; y < faces.length; y++) {
              ...
              // java loads this in the wrong order
   ...
   * @return the lines read or null if fail
   */
  private static ArrayList<String> readTextFile(String path) {
    try (BufferedReader br = new BufferedReader(new FileReader(path))) {
      ...
      warning("readTextFile", "Exception thrown when reading " + path + " ");
    }
    return null;
  }

  /**
   * Display warning message for a loading issue
   *
   * @param mthd the method the failure happened in
   * @param msg the message to be displayed given failure
   */
