...
/**
 * @author Sujith Ramanathan <br>
 *     <br>
 *     <li>ACR122U Reader Helper Utility is used to read,write,encrypt and decrypt Mifare Classic 1k
 *         Cards only.
 */
public class ACR122UReaderHelper {
  ...
  private Logger logger = LoggerContext.getContext().getLogger(ACR122UReaderHelper.class);
  ...
  /**
   * @return boolean
   *     <li>This method will return true, If it is connected to ACR122U Reader.
   */
   ...
   *     <li>This method is used connect card terminal.
   */
  public boolean connectCard(String connectionProtocol) throws CardException {
    if (null != terminal && terminal.isCardPresent()) {
      ...
      // throw new CardDataException("Please run connectReader() method first or please connect
      // ACR122U reader properly");
      return false;
    }
  }
   ...
   * @param blockNo
   * @return byte[]
   *     <p>This method is used to read the card block using default key which is an unique key to
   ...
   * @param blockNo
   * @return byte []
   *     <li>This method has 3 parameters, user defined authentication key, Authentication command
   *         for Key A or Key B, card block to read and return byte Array.
   */
   ...
   * @param data
   * @return byte[]
   *     <li>This method has 4 parameters, user defined authentication key, Authentication command
   *         for Key A or Key B, card block to write, byte data of 16 bytes to write in the card.
   */
  public byte[] writeDataIntoCard(byte[] authKeyData, byte[] authKeyCmd, int blockNo, byte[] data) {
    if (null == authKeyData || authKeyData.length != 6)
      throw new CardDataException(
          "Insufficient authKeyData Param length. Length should be 6 in size");
    ...
    for (int i = 0; i < data.length; i++) {
      if (i < 5) byteDataArr[i] = writeCardCmd[i];
   ...
   * @return byte[]
   *     <li>This method is used to encrypt card using default key. Once the card is encrypted the
   *         key will be stored on Key B side.
   *     <li>We have used access bits to grant read and write access internally to the reader. At
   *         the same time if we try to read this block the output will be 0.
   */
  public byte[] encryptCardUsingDefaultKey(
      byte[] existingAuthKey, byte[] authCmdToAccess, byte[] newAuthKey, int trailerBlockNo) {

    if (null == existingAuthKey || existingAuthKey.length != 6)
      throw new CardDataException(
          "Insufficient existingAuthKey Param length. Length should be 6 in size");
    ...
    try {
      ...
      if (tBlock != trailerBlockNo)
        throw new CardDataException(
            "Invalid Trailor Block ".concat(String.valueOf(trailerBlockNo)));
    ...
    for (int i = 0, j = 5; i < existingAuthKey.length; i++, j++) {
      if (i < 5) authKeyData[i] = createManualAuthenticationKey[i];
    ...
    for (int i = 0; i < 21; i++) {
      if (i < 15) updateSecKey[i] = writeOnKeyBAndInheritAs0Cmd[i];
      else {
   ...
   * @return byte[]
   *     <li>This method has 3 parameters, user defined authentication key, Authentication command
   *         for key A or key B, trailer block number where the authentication key is stored.
   *     <li>This method will override the current authentication key with default key(i.e., The key
   ...
   * @param
   * @return byte Array
   *     <li>This method will read card code from manufacturer block and returns the same.
   */
  public byte[] getUID() {
  ...
  private byte[] sendCommand(byte[] apduCmd) {
    if (null == card) throw new CardDataException("Place card on the reader");
   ...
   * @param apduCmd
   * @return ResponseApdu
   *     <li>This method is used to send Apdu command directly and will return ResponseAPDU.
   */
   ...
   * @param interval
   * @throws CardException
   *     <li>This method will make the application wait for card to be present on the reader.
   */
   ...
   * @param blockNo
   * @return byte[]
   *     <p>This method is used for internal authentication purpose.
   */
  private byte[] doAuthentication(byte[] authKeyData, byte[] authKeyCmd, int blockNo) {
    ...
    for (int i = 0, j = 5; i < authKeyData.length; i++, j++) {
      if (i < 5) encryptedKey[i] = createManualAuthenticationKey[i];
