...
public class EquipmentPurchaseRequestListController implements Initializable, IMenuAccess {
  ...
  private final String requestPageURL =
      "edu/wpi/cs3733/D22/teamZ/views/EquipmentPurchaseRequest.fxml"; // change

  // List of identifiers for each
  private final String[] identifiers = {
    "ID", "Equipment", "Issuer", "Handler", "Status", "Target Location", "Purchase Method" // change
  };

  // Columns to be represented by the table
  private final List<String> visibleColumns =
      List.of("ID", "Equipment", "Status", "Issuer"); // change

  // Retriever functions. Correspond to visible columns.
  private final List<RequestRowFunc> retrievers =
      List.of(row -> row.id, row -> row.equipment, row -> row.status, row -> row.issuer);

  private final List<RequestFunc> detailRetrievers =
      List.of(
          request -> request.getRequestID(),
          request -> request.getEquipmentType(),
          ...
          request -> request.getPaymentMethod());

  // List of requests that represents raw data
  private List<EquipmentPurchaseRequest> rawRequests; // change
  ...
  public EquipmentPurchaseRequestListController() throws SQLException { // change
  ...
  public String getMenuName() {
    return "Equipment Purchase Request List";
  } // change
  ...
  public void refreshClicked(ActionEvent event) throws SQLException {
  ...
  public void createRRList() {
    ...
    for (EquipmentPurchaseRequest request : rawRequests) { // change
  ...
  public void loadRow(String reqID) {
    ...
    EquipmentPurchaseRequest selectedReq = getRequestFromID(reqID); // change
  ...
  public void loadRequests() throws SQLException {
    rawRequests = FacadeDAO.getInstance().getAllEquipmentPurchaseRequests(); // change
  }

  public EquipmentPurchaseRequest getRequestFromID(String MeqID) { // change
    return FacadeDAO.getInstance().getEquipmentPurchaseRequestByID(MeqID); // change
  }
  ...
  public void exportToCSV(ActionEvent actionEvent) {
    ...
    File defaultFile = facadeDAO.getDefaultEquipmentPurchaseRequestCSVPath(); // change
    ...
    facadeDAO.exportEquipmentPurchaseRequestsToCSV(file); // change
  }
  ...
  private static class RequestRow {
    ...
    public RequestRow(String newId, String newType, String newIssuer, String newStatus) {
      ...
      equipment = new SimpleStringProperty(newType);
  ...
  private interface RequestFunc {
    String call(EquipmentPurchaseRequest request); // change
  }
}
